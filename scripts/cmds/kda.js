...
module.exports = {
  config: {
    ...
    countDown: 30,
    role: 2,
    shortDescription: "Get random kanda video",
    longDescription: "",
    category: "𝟭𝟴+",
    guide: "{p}{n}",
  },
  ...
  onStart: async function({ message, api, event }) {
    ...
    const loadingMessage = await message.reply("হাত মারা রিয়েল হাত মারতে থাকো 🐸");
    ...
    const link = [
      ...
      /* ... তোমার দেয়া লিংকগুলো সব এখানে থাকবে ... */
      "https://drive.google.com/uc?export=download&id=1zRAFPp3sMPOlVyhoEPnHflRpiRe6C2pt"
    ];

    // এখনও যেসব ভিডিও পাঠানো হয়নি সেগুলো ফিল্টার করো
    let availableVideos = link.filter(video => !this.sentVideos.includes(video));

    // যদি সব ভিডিও পাঠানো হয়ে যায়, লিস্ট রিসেট করো
    if (availableVideos.length === 0) {
      this.sentVideos = [];
      availableVideos = [...link];
    }

    // র্যান্ডম একটি ভিডিও বেছে নাও
    ...
    try {
      await message.reply({
        body: 'তোগো বউরে লাভ উ 😘',
        attachment: await global.utils.getStreamFromURL(randomVideo),
      });

      // ৫০ সেকেন্ড পরে লোডিং মেসেজ ডিলিট করবে
      setTimeout(() => {
        api.unsendMessage(loadingMessage.messageID).catch(() => {});
      }, 50000);
    } catch (error) {
      console.error("Video sending error:", error);
      message.reply("ভিডিও পাঠানো যায়নি, পরে চেষ্টা করুন।");
      api.unsendMessage(loadingMessage.messageID).catch(() => {});
    }
  },
};
